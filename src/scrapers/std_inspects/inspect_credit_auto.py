...
KEY = "CREDIT_AUTO"
URL = "https://www.stb.com.tn/fr/simulateurs/simulateurs-credit/simulateur-credit-auto/"

def fill(page, profile: dict):
    typecredit = profile.get("std_typecredit", "PDA02_01")
    common.select_option(page, "#TypeCredit", typecredit)

    montant = int(float(profile.get("montant_pret_demande", 50000)))
    ...
    duree_mois = int(profile.get("duree_mois", 48))
    duree_ans = max(1, min(7, int(round(duree_mois / 12))))
